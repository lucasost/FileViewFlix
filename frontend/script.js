document.addEventListener("DOMContentLoaded", function () {
  fetch("http://127.0.0.1:8000/files")
    ...
    .then((files) => {
      ...
      const commonPrefix2 = findCommonPathPrefix(files);
      headerTitle.textContent = `Diretório Raiz: ${commonPrefix2}`;
      ...
      function updateDirectoryCompletionStatus(directoryPath) {
        const dirLink = document.querySelector(
          `a[data-path="${directoryPath}"]`
        );
        if (!directories[directoryPath]) return;
        const allCompleted = directories[directoryPath].every(
          (file) => localStorage.getItem(file) === "completed"
        );
        if (allCompleted) {
